...
namespace App.Persistence
{
        ...
        : IGenericRepository<T, TId> where T : BaseEntity<TId> where TId : struct
    {
        ...
        public Task<List<T>> GetAllPagedAsync(int pageNumber, int pageSize, Expression<Func<T, object>> includeProperty = null)
        {
            if(includeProperty is null)
                return _dbSet.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync();
            else
                return _dbSet.Include(includeProperty).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync();
        }
